...
class CalendarEventCard(MDCard):
    ...
    def __init__(
        ...
        **kwargs
        ...
        description = re.sub(
            "(https://.*?)(?=\s|$)", 
            "[u][ref=\\1][color={0}]\\1[/color][/ref][/u]".format(colors['LightGreen']['500']), 
            description
        )
        ...
        padder = MDBoxLayout(orientation="vertical", adaptive_height=True, padding=(dp(70), 0, dp(10), 0))
...
class CalendarEventContent(MDBoxLayout):
    ...
    def init_ui(self):
        ...
        async def set_items():
            ...
            def filter_fn(event): 
                ...
                return all(
                    (
                        len(self.ids.stack.children) > 0 and [
                            fnmatch(
                                event['summary'].decode(), 
                                chip.text
                            ) for chip in self.ids.stack.children 
                        ] or [False]
                    ) + [
                        event['dtstart'] >= self.start if isinstance(self.start, datetime) else True
                    ] + [
                        event['dtend'] <= self.end if isinstance(self.end, datetime) else True 
                    ]
                )

            # iterate over matching events
            for cnt, event in enumerate(
                list(
                    filter(
                        filter_fn, 
                        self.events
                ...
                # update dialog title
                self.dialog.title = re.compile('\(\d+\)').sub(f'({cnt})', self.dialog.title)
                # create event widget
                card = CalendarEventCard(
                    ...
                    description=event['description'].decode() if event.get('description') else '',
                )
    ...
    def create_search_tag(self):
        ...
        if '*' not in search_phrase[::len(search_phrase) > 1 and len(search_phrase)-1 or 1]:
            search_phrase = '*'+search_phrase+'*' if search_phrase else '*'
        # add filter if not present yet
        if not any([stacked.text == search_phrase for stacked in self.ids.stack.children]):
        ...
        if boundary in ("start", "end"):
            ...
            def get_time():
                ...
                def on_save_time(instance:MDTimePicker, time:datetime.time):
                    ...
                    nonlocal self
                    setattr(self, boundary, getattr(self, boundary).replace(
                        hour=time.hour, 
                        minute=time.minute
                    ))
            ...
            if self.start.date() != self.end.date():
                ...
                def on_save_date(instance:MDDatePicker, value:datetime.date, date_range:list):
                    """
                    Action to be performed on selection.
